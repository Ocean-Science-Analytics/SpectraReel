  ...
  sidebarLayout(
    ...
    sidebarPanel(width=4, class="sidebar-panel",
      ...
      fluidRow(
        ...
        column(6, numericInput("freq_max","Max Freq (Hz)",8000, min=100,  max=96000,step=100))
      ),
      ...
      checkboxGroupInput("visual_opts","Visual Options",
        choices=c("Shade played region"="shade","Show filename title"="title",
                  "Show elapsed time"="time_label","Show colorbar"="colorbar"),
        selected=c("title","time_label")),
      ...
      div(class="sv-card",
        ...
        div(class="preview-wrap",
          plotOutput("preview_plot",
            height="340px",
            click="plot_click")           # click to seek
        ),
...
server <- function(input, output, session) {
  ...
  output$preview_plot <- renderPlot({
    # Depend on playhead for live animation
    t_now <- rv$playhead
    s     <- settings()
    spec  <- tryCatch(spec_data(), error=function(e) NULL)
    ...
    top_mar <- if(s$show_title || s$show_time) 2.6 else 1.2
    rt_mar  <- if(s$colorbar) 5.2 else 1.2

    par(bg=s$bg_color, mar=c(3.4,3.8,top_mar,rt_mar),
        col.axis=s$text_color, col.lab=s$text_color,
        fg=s$text_color, family="sans")
    ...
    axis(1,col=s$text_color,col.ticks=s$text_color,col.axis=s$text_color,cex.axis=.78,tcl=-.25)
    axis(2,col=s$text_color,col.ticks=s$text_color,col.axis=s$text_color,cex.axis=.78,tcl=-.25,las=1)
    ...
    if (s$show_title)
      mtext(rv$audio_name, side=3,
            line=(if(s$show_time) 1.3 else .3),
...
Options      : %s",
      ...
      paste(Filter(Negate(is.null), list(
              ...
              if(s$show_time)"time", if(s$colorbar)"colorbar")),
            collapse=", ")))
  })
